...
function createUser($conn , $name , $email , $username , $pwd ,$nic)
{
    $sql = "INSERT INTO users(usersName , usersEmail, usersUid , usersPwd ,NIC ) values (?,?,?,?,?); ";
    $stmt = mysqli_stmt_init($conn);
    ...
    mysqli_stmt_bind_param($stmt, "ssssi", $name, $email , $username , $hashedPwd, $nic);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
$uidExist = uidExists( $conn , $username , $username);
    ...
    if($checkPwd === true)
    {
        ...
        $_SESSION["usersEmail"] = $uidExist ["usersEmail"];
